    ...
    constructor(data, client) {
        ...
        if (data.message_snapshots && this.messageReference) {
            this.messageSnapshots = data.message_snapshots.map((snapshot) => {
                snapshot.message._no_author = true;
        ...
        switch(this.type) {
            ...
            case MessageTypes.RECIPIENT_ADD: {
                data.content = `${this.author && this.author.mention} added <@${data.mentions[0].id}>.`;
                break;
            }
            case MessageTypes.RECIPIENT_REMOVE: {
                if(this.author.id === data.mentions[0].id) {
                    data.content = `@${this.author && this.author.username} left the group.`;
                } else {
                    data.content = `${this.author && this.author.mention} removed @${data.mentions[0].username}.`;
                }
                break;
            }
            case MessageTypes.CALL: {
                if(data.call.ended_timestamp) {
                    ...
                    if(data.call.participants.includes(this._client.user.id)) {
                        data.content = `You missed a call from ${this.author && this.author.mention}.`;
                    } else {
                        data.content = `${this.author && this.author.mention} started a call.`;
                    }
                } else {
                    ...
                    data.content = `${this.author && this.author.mention} started a call. — Join the call.`;
                }
                break;
            }
            case MessageTypes.CHANNEL_NAME_CHANGE: {
                data.content = `${this.author && this.author.mention} changed the channel name: ${data.content}`;
                break;
            }
            case MessageTypes.CHANNEL_ICON_CHANGE: {
                data.content = `${this.author && this.author.mention} changed the channel icon.`;
                break;
            }
            case MessageTypes.CHANNEL_PINNED_MESSAGE: {
                data.content = `${this.author && this.author.mention} pinned a message to this channel. See all the pins.`;
                break;
            }
            case MessageTypes.GUILD_MEMBER_JOIN: {
                data.content = SystemJoinMessages[~~(this.createdAt % SystemJoinMessages.length)].replace(/%user%/g, this.author && this.author.mention);
                break;
            }
            case MessageTypes.USER_PREMIUM_GUILD_SUBSCRIPTION: {
                data.content = `${this.author && this.author.mention} just boosted the server!`;
                break;
            }
            ...
            case MessageTypes.USER_PREMIUM_GUILD_SUBSCRIPTION_TIER_3: {
                data.content = `${this.author && this.author.mention} just boosted the server! ${this.channel.guild ? this.channel.guild.name : data.guild_id} has achieved **Level ${this.type - 8}!**`;
                break;
            }
            case MessageTypes.CHANNEL_FOLLOW_ADD: {
                data.content = `${this.author && this.author.mention} has added ${data.content} to this channel`;
                break;
            }
